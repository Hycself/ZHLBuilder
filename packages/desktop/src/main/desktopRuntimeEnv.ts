...
// ZHLBuilder fork：不能与本机官方 ZCode 共用应用名 —— userData (%APPDATA%\ZCode)、
// 单实例锁、会话与凭据全部按 app name 隔离，共用即冲突。
export const runtimeApplicationName =
  readRuntimeEnvOverride("ZCODE_DESKTOP_APPLICATION_NAME") ??
  (isLocalDevelopmentRuntime ? "ZHLBuilder Dev" : isPreviewPackagedRuntime ? "ZHLBuilder Preview" : "ZHLBuilder");
