...
// 桌面打包态的 agent 运行时资产：把 agent 的 JS bundle（zhlbuilder.cjs）放进 bundled-agents/<platform>/glm，
...
// - app-server 命令路径不会加载 @zhlbuilder/tui，所以这里天然不打包 TUI。
//
...
const cliBundlePath = resolve(repoRoot, "apps/zcode-cli/packages/cli/dist/zhlbuilder.cjs");
...
const pnpmRunEnv = {
  ...
  // 子 workspace 不能解析根 workspace 的 @zhlbuilder/shared，Docker/web app 打包会因此卡在插件 runtime 构建。
...
// zhlbuilder.cjs / .node-bundle-meta.json 的落点由 stage-agent-bundle.mjs 自己解析（同源）。
// node_repl 宿主抽成独立包
// @zhlbuilder/node-repl-host 之后，browser-use 不再产出 dist/mcp/server.js，CUA 资产
// （docs/computer-use.md、scripts/computer-use-client.mjs）也已归 @zhlbuilder/zcode-cua-plugin。
...
const officialPluginPackages = [
  {
    ...
    // @zhlbuilder/node-repl-host（见上方常量注释）。
...
// 在 zhlbuilder.cjs 旁找 packages/bundled-skills 并原地读取。漏 stage 它，桌面包的 /workflow 会展开成
...
function buildCliBundle() {
  ...
  // 复用仓库根脚本（turbo build:desktop-agent --filter=@zhlbuilder/cli），命中缓存时几乎瞬时。
...
// Electron 生产包只带 resources/glm/zhlbuilder.cjs 时，app-server 进程的
// __dirname 附近没有官方插件目录，启动时 seed 找不到 source，用户侧不会自动得到内置插件。
// 这里把官方插件按 bootstrap 的 rootCandidates 期望放到 glm/packages/*-plugin，
// 让 Electron Node 运行 zhlbuilder.cjs 时复用同一套 filesystem seed 逻辑。
// browser-use runtime 的声明生成依赖 @zhlbuilder/core/dist。CI 干净检出没有该产物，
