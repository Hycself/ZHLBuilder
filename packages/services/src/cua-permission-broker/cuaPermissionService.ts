...
// to @zhlbuilder/zcode-cua/src/broker/ports.ts. The descriptor registration itself stays in
// services (host control plane — depends on services' createServiceDescriptor
// + @zhlbuilder/shared ServiceChannels), so services internal callers (node.ts,
// accessor.ts, services/index.ts) and ui consumers (via @zhlbuilder/services root
...
// at compile time; Vite never resolves @zhlbuilder/zcode-cua for these).
import type {
  CuaPermissionState,
  CuaPermissionStatus,
...
} from "@zhlbuilder/zcode-cua/broker";
...
} from "@zhlbuilder/zcode-cua/broker/ports";
