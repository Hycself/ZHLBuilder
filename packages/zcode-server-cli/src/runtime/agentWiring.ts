...
export function createReleaseAgentWiring(
  ...
  return {
    ...
    ZCODE_AGENT_SERVER_ARGS_JSON: JSON.stringify([
      join(runtimeRoot, "zhlbuilder.cjs"),
      "app-server",
      "--stdio",
    ]),
  };
 ...
 * `zhlbuilder.cjs` 注入为 agent 启动命令；env 覆盖是该解析链的最高优先级，因此显式配置的
 * `ZCODE_AGENT_SERVER_COMMAND` 永远优先，开发态（入口同目录无 zhlbuilder.cjs）不受影响。
 */
export async function resolveBundledAgentWiring(
  ...
  const bundlePath = join(entryDir, "zhlbuilder.cjs");
  try {
