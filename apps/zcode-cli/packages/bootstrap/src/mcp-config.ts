...
export { ZCODE_CUA_BROKER_SOCKET_ENV_KEY as ZCODE_CUA_BROKER_SOCKET_ENV } from "@zhlbuilder/shared";
...
function isZCodeCuaStdioServer(
  ...
  // (非 zcode-cua)、args 是 [zhlbuilder.cjs, __zcode-plugin-host, server.js]（非 zcode-cua package arg），
  ...
  // 判定与 desktop/services 共用 @zhlbuilder/shared 的单一事实源，避免两条注入入口漂移。
