...
// 保持「只依赖 @zhlbuilder/shared」的纪律（与共享 reducer 不得反向依赖 contracts 同一姿态）；
// 顺带让 app-events.ts 的 switch 保持在 max-lines 之内。
import type { DynamicWorkflowRunProgressPayload } from "@zhlbuilder/contracts";
import type { WorkflowRunProgressEnvelope } from "@zhlbuilder/shared/zcode-protocol-v4";
